...
classDiagram
    ...
    class RepairOrder {
        ...
        +userLogisticsImage: String
        +officialLogisticsNo: String
        +officialLogisticsImage: String
        +createdAt: LocalDateTime
        +receivedAt: LocalDateTime
        +repairedAt: LocalDateTime
        +shippedAt: LocalDateTime
        +submit(description: String, logisticsNo: String, image: String): void
        +confirmReceipt(): void
        +completeRepair(notes: String): void
        +shipBack(logisticsNo: String, image: String): void
...
| UpdateMicroclimate | climateZone: String | MicroclimateUpdated | 更新微气候区域设置 |
...
#### 4.2.7 RepairOrder（返修工单）

| 属性 | 类型 | 描述 |
| :--- | :--- | :--- |
| repairOrderId | UUID | 返修单ID |
| deviceId | UUID | 设备ID |
| userId | UUID | 申请人ID |
| orderNo | String | 工单编号 (RO-xxx) |
| status | RepairStatus | 状态 (Applied, Received, Repaired, Shipped, Completed) |
| issueDescription | String | 故障描述 |
| userLogisticsNo | String | 用户寄件单号 |
| userLogisticsImage | String | 用户寄件凭证 |
| officialLogisticsNo | String | 官方回寄单号 |
| officialLogisticsImage | String | 官方回寄凭证 |
| createdAt | LocalDateTime | 创建时间 |

| 命令 | 参数 | 事件 | 业务规则 |
| :--- | :--- | :--- | :--- |
| SubmitRepair | deviceId: UUID, description: String, logisticsNo: String, image: String | RepairSubmitted | 验证设备归属权 |
| ConfirmReceipt |  | RepairReceived | 运维确认收货 |
| CompleteRepair | notes: String | RepairCompleted | 维修完成 |
| ShipBack | logisticsNo: String, image: String | RepairShipped | 录入回寄信息 |
| ConfirmDelivery |  | RepairDelivered | 用户确认收货 |

#### 4.2.8 AssetCompensate（资产赔付）

| 属性 | 类型 | 描述 |
| :--- | :--- | :--- |
| compensateId | Long | 赔付ID |
| deviceId | Long | 关联设备ID |
| compensationAmount | BigDecimal | 赔付金额 |
| depositDeduction | BigDecimal | 押金抵扣 |
| needToPay | BigDecimal | 需补差价 |
| status | String | 状态 |
| createdAt | LocalDateTime | 创建时间 |
...
| 命令 | 参数 | 事件 | 业务规则 |
| :--- | :--- | :--- | :--- |
| CalculateCompensation | | CompensationCalculated | 计算赔付金额 |
| ConfirmPayment | | PaymentConfirmed | 确认赔付并解绑 |
...
| DeviceControlService | 设备连接上下文 | 设备远程控制 | DeviceRepository |
| RepairOrderService | 交付运维上下文 | 返修工单管理 | RepairOrderRepository, DeviceRepository |
...
| RepairOrderRepository | RepairOrder | save(order: RepairOrder), findByUserId(userId: UUID), findByOrderNo(orderNo: String), findByPage(page: Integer, pageSize: Integer, status: String) |
...
#### 9.2.10 repair_orders
| 字段名 | 数据类型 | 约束 | 描述 |
| :--- | :--- | :--- | :--- |
| repair_order_id | uuid | primary key | 返修单ID |
| device_id | uuid | foreign key | 设备ID |
| user_id | uuid | foreign key | 用户ID |
| order_no | varchar(50) | unique not null | 工单编号 |
| status | varchar(20) | not null | 状态 |
| issue_description | text | | 故障描述 |
| user_logistics_no | varchar(50) | | 用户寄件单号 |
| user_logistics_image | varchar(200) | | 用户寄件凭证 |
| official_logistics_no | varchar(50) | | 官方回寄单号 |
| official_logistics_image | varchar(200) | | 官方回寄凭证 |
| created_at | timestamp | not null | 创建时间 |
| received_at | timestamp | | 收货时间 |
| repaired_at | timestamp | | 维修完成时间 |
| shipped_at | timestamp | | 寄回时间 |
| updated_at | timestamp | not null | 更新时间 |
...
- 2025-12-16: 完善模拟服务设计
- 2025-12-16: 补充预留接口设计
- 2025-12-22: 同步API定义与前端实现，补充AssetCompensate聚合根、微气候配置与防拆处理逻辑
