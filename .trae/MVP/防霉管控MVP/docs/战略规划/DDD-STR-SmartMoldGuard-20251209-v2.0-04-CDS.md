...
1. 当设备为**押金式租赁**模式时，资产赔付需遵循"**先押金，后补差**"原则：  
    1.1 运维服务在 `CheckRentalStatus(DeviceId)` 时，必须同时读取该设备绑定的押金余额（例如 ¥200）。  
    1.2 运维服务在 `CalculateCompensation(...)` 中计算**应赔付金额**（例如按剩余租期或设备残值标准得到 ¥150）。  
    1.3 系统应优先从押金中扣减：`应扣押金 = min(应赔付金额, 押金余额)`，并记录"押金抵扣金额"字段。  
    1.4 若 `应赔付金额 > 押金余额`，则计算 `需补差金额 = 应赔付金额 - 押金余额`，通过小程序向用户发起在线补差支付请求。  
2. 赔付完成后的押金处理：  
    2.1 若押金扣减后仍有余额，且用户完成补差支付或设备已正常归还，则剩余押金需原路退回给用户。  
    2.2 若用户拒绝补差支付且设备未归还/已确认报废，则该设备应标记为 `AssetLost`，关联工单保持 `Pending` 或进入争议处理队列，直到人工介入。  
3. 整个押金扣减与补差支付过程必须与 `AssetCompensate` 聚合状态保持一致：  
    3.1 仅当赔付记录处于 `CompensationPending` 状态时，才允许执行押金扣减与补差支付。  
    3.2 一旦补差支付完成并生成 `CompensationPaid` 事件，对应工单应自动进入已关闭状态，防止重复扣款。

### SCEN-003：酒店轻量化风险监测 (Hotel Lightweight Monitoring) [B端]

#### 2.5 业务背景
针对仅安装传感器的酒店/民宿场景（无控制面板），系统**每日早晚两次 (8:00 & 20:00)** 自动生成按风险排序的房间清单，指导工作人员进行针对性通风或除湿，替代人工逐个巡检。
