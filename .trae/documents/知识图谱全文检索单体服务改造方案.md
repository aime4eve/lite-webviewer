...
graph TD
    ...
    subgraph "单体后端服务 (Monolithic Backend)"
        ...
        subgraph "模块划分 (Python Modules)"
            FlaskApp --> APIBlueprint[API 蓝图模块]
        ...
        ServiceLayer -.-> AsyncTasks[异步任务队列]
    end
    ...
    subgraph "本地数据存储 (Docker Compose)"
        FlaskApp & TaskWorker --> LocalFS[本地文件存储]
        FlaskApp & TaskWorker --> ES_Single[Elasticsearch]
        FlaskApp & TaskWorker --> Milvus_Lite[Milvus Lite]
        FlaskApp & TaskWorker --> Nebula_Single[NebulaGraph]
        FlaskApp & TaskWorker --> Redis[Redis]
        FlaskApp & TaskWorker --> SQLite[SQLite/MySQL]
    end
